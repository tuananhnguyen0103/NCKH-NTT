...
const Xlsxadd = () => {
    ...
    const [kiHoc, setKiHoc] = useState(null)
    ...
    const onSubmitHandler = (event) => {
        ...
        readXlsxFile(imagefile).then((rows) => {
            console.log(JSON.stringify(rows))
             ...
             axios.post(process.env.REACT_APP_BACKEND_URL + '/teacher/InsertScoreByYearAndSemmesterIdHocSinh_XLSX?token='+token, 
             { 
                    ...
                    <form className="new-added-form" onSubmit={onSubmitHandler}>
                        <div className="row">
                            ...
                            <div className="col-3-xxxl col-xl-3 col-lg-3 col-12 form-group">
                                <input onChange={value=>setKiHoc(value.target.value)} type="text" placeholder="Chọn kì học" className="form-control" />
                            </div>
                            <div className="col-4-xxxl col-xl-4 col-lg-3 col-12 form-group">
                                <input onChange={value=>setNamHoc(value.target.value)} type="text" placeholder="Chọn năm học" className="form-control" />
                            </div>
