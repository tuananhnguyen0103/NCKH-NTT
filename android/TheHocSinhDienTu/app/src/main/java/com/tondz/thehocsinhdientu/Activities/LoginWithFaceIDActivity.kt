...
class LoginWithFaceIDActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_login_with_face_idactivity)
        if (DBHelper(applicationContext).faceID != null) {
            faceID = DBHelper(applicationContext).faceID
            Common.PREDICTS = Common.toList(faceID.labels)
        }
    ...
    private fun login(id: String, password: String) {
        Log.e("Login","hien")
        val dialog = ProgressDialog(this@LoginWithFaceIDActivity)
        dialog.setMessage("Đang đăng nhập")
        dialog.show()
        val map = HashMap<String, String>()
        map["id"] = id
        map["password"] = password
        api.Login(map)!!.enqueue(object : Callback<LoginModel?> {
            override fun onResponse(call: Call<LoginModel?>, response: Response<LoginModel?>) {
                if (response.code() == 200) {
                    Common.TOKEN = response.body()!!.token
                    Common.ID = id
                    Common.PASSWORD = password
                    if (response.body()!!.hocSinh != null) {
                        Log.e(
                            "Login", response.body()!!.hocSinh.hoTen
                                    + ""
                        )
                        Common.HOC_SINH = response.body()!!.hocSinh
    ...
    private fun duDoan(bitmap: Bitmap) {
        ...
        if (acc(pred, prob) > 0.5) {
            ...
            if (flag&&!flagLogin) {
                ...
                login(faceID.idHocSinh, faceID.password)
            }
